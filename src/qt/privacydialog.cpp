                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTHE ought to be enough for anybody." - Bill Gates, 2017
    ui->zTHEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTHESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTHE->setEnabled(false);
        ui->pushButtonMintzTHE->setToolTip(tr("zTHE is currently disabled due to maintenance."));

        ui->pushButtonSpendzTHE->setEnabled(false);
        ui->pushButtonSpendzTHE->setToolTip(tr("zTHE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTHEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTHE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTHE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTHE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTHE in ") +
...
void PrivacyDialog::on_pushButtonSpendzTHE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTHE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTHE
        sendzTHE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTHE
    sendzTHE();
}

void PrivacyDialog::on_pushButtonZTHEControl_clicked()
{
    ZTHEControlDialog* zTHEControl = new ZTHEControlDialog(this);
    zTHEControl->setModel(walletModel);
    zTHEControl->exec();
}

void PrivacyDialog::setZTHEControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTHESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTHE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TheNode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTHEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTHE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " THE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTHE</b>";
    ...
    // use mints from zTHE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTHEControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTHEControlDialog::GetSelectedMints();
    }
    ...
    // Clear zTHE selector in case it was used
    ZTHEControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTHE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " THE, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTHE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTHE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTHE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTHE "));
    ui->labelzTHEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTHESyncStatus->setVisible(fShow);
}
