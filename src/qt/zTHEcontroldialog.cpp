...
#include "ui_zTHEcontroldialog.h"
...
std::list<std::string> ZTHEControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTHEControlDialog::listMints;

ZTHEControlDialog::ZTHEControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTHEControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTHEControlDialog::~ZTHEControlDialog()
{
    delete ui;
}

void ZTHEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTHEControlDialog::updateList()
{
...
void ZTHEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTHEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTHE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTHEControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTHEControlDialog::GetSelectedMints()
{
...
void ZTHEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
